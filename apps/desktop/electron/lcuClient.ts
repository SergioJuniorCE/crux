...
export type ChampSelectSessionPayload = {
...
function mergeSummonerIdentity(
  summoner: LcuSummoner,
  chatMe: LcuChatMe | null,
): LcuSummoner {
...
function summonerFromChatMe(chatMe: LcuChatMe): LcuSummoner | null {
  ...
  const displayName =
    gameName && tagLine
      ? `${gameName}#${tagLine}`
      : cleanString(chatMe.name) || gameName;
...
function parseCredentialsFromCommandLine(
  cmdLine: string,
): LcuCredentials | null {
  ...
  try {
    ...
    for (const line of lines) {
      try {
        ...
        const credentials = obj.CommandLine
          ? parseCredentialsFromCommandLine(obj.CommandLine)
          : null;
        const installDir = obj.ExecutablePath
          ? path.dirname(obj.ExecutablePath)
          : null;
        if (credentials) {
...
async function findCredentials(): Promise<LcuCredentials | null> {
  if (process.platform === "win32") {
    const { credentials, installDir } =
      await findCredentialsFromProcessWindows();
...
async function getCredentialsOrThrow(): Promise<LcuCredentials> {
...
export async function getCurrentSummonerFromClient(): Promise<CurrentSummonerPayload> {
  ...
  const region = await lcuRequest<LcuRegionLocale>(
    creds,
  ...
  return {
    summoner: mergeSummonerIdentity(summoner, chatMe),
    platform,
    regionCode,
  };
}

export async function getChampSelectSessionFromClient(): Promise<ChampSelectSessionPayload> {
  const creds = await getCredentialsOrThrow();
  return lcuRequest<ChampSelectSessionPayload>(
    creds,
